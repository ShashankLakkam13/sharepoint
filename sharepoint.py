...
credentials directly in the source file.  To keep this repository safe the
script now expects credentials and runtime settings to be supplied explicitly
through command-line arguments when it is executed.
"""
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tenant-id",
        required=True,
        help="Azure Active Directory tenant ID (GUID).",
    )
    parser.add_argument(
        "--client-id",
        required=True,
    ...
    parser.add_argument(
        "--client-secret",
        help="Azure AD application client secret (required for application auth).",
    )
    parser.add_argument(
        "--hostname",
        required=True,
        help="SharePoint Online hostname, e.g. contoso.sharepoint.com.",
    )
    parser.add_argument(
        "--site-path",
        default="/",
        help="SharePoint site path, e.g. /sites/Example (defaults to /).",
    )
    parser.add_argument(
        "--auth-mode",
        choices=["device", "application"],
        default="device",
        help="Authentication mode: 'device' for device code flow, 'application' for app-only.",
    )
    parser.add_argument(
        "--download-dir",
        default="./aspx_dump",
        help="Directory to store downloaded .aspx files (default: ./aspx_dump).",
    )
    parser.add_argument(
        "--export-csv",
        default="./aspx_index.csv",
        help="Path to the CSV file that lists discovered pages (default: ./aspx_index.csv).",
    )
    parser.add_argument(
        "--no-download-aspx",
        dest="download_aspx",
        action="store_false",
        help="Disable downloading of .aspx files (metadata will still be recorded).",
    )
    parser.add_argument(
        "--no-search-fallback",
        dest="search_fallback",
        action="store_false",
        help="Disable Microsoft Graph search fallback when traversal finds nothing.",
    )
    parser.set_defaults(download_aspx=True, search_fallback=True)
...
if __name__ == "__main__":
    ...
    if args.auth_mode == "application" and not args.client_secret:
        fail("--client-secret is required when --auth-mode is 'application'.")

    config = {
        "AUTH_MODE": args.auth_mode,
        "AZURE_TENANT_ID": args.tenant_id.strip(),
        "AZURE_CLIENT_ID": args.client_id.strip(),
        "AZURE_CLIENT_SECRET": (args.client_secret or "").strip(),
        "SPO_HOSTNAME": args.hostname.strip(),
        "SPO_SITE_PATH": normalize_site_path(args.site_path.strip()),
        "DOWNLOAD_ASPX": args.download_aspx,
        "DOWNLOAD_DIR": pathlib.Path(args.download_dir),
        "EXPORT_CSV": pathlib.Path(args.export_csv),
    ...
    if missing:
        fail("Missing required configuration values: " + ", ".join(missing))
