...
To keep deployment simple for environments without configuration tooling, all
required values are provided directly inside this script.  Edit the
``USER_SETTINGS`` block below and replace the placeholder values with your own
tenant, application, and SharePoint site details before running the crawler.
"""
...
# Update the values in this dictionary with your own details before running the
# script.  Fields marked with "TODO" must be replaced.  Leave optional fields as
# they are if the defaults work for your environment.
USER_SETTINGS = {
    "AUTH_MODE": "device",  # Change to "application" when using client credentials
    "AZURE_TENANT_ID": "",  # TODO: Azure AD tenant ID (GUID)
    "AZURE_CLIENT_ID": "",  # TODO: Azure AD application (client) ID
    "AZURE_CLIENT_SECRET": "",  # TODO: Required only when AUTH_MODE is "application"
    "SPO_HOSTNAME": "",  # TODO: SharePoint hostname, e.g. "contoso.sharepoint.com"
    "SPO_SITE_PATH": "/sites/Example",  # TODO: Site path starting with "/" (use "/" for the root site)
    "DOWNLOAD_ASPX": True,  # Set to False to skip downloading page content
    "DOWNLOAD_DIR": "./aspx_dump",  # Folder for downloaded pages
    "EXPORT_CSV": "./aspx_index.csv",  # CSV file that lists discovered pages
    "SEARCH_FALLBACK": True,  # Set to False to disable the Microsoft Graph search fallback
}


def normalize_site_path(path: str) -> str:
    """Ensure the SharePoint site path starts with a slash."""
...
def coerce_to_bool(value, *, default: bool) -> bool:
    """Convert a ``USER_SETTINGS`` value to ``bool``."""
...
def build_config_from_user_settings(settings: Dict[str, object]) -> Dict[str, object]:
    """Validate ``USER_SETTINGS`` and construct the runtime configuration."""

    auth_mode = str(settings.get("AUTH_MODE", "device")).strip().lower() or "device"
    if auth_mode not in {"device", "application"}:
        raise ValueError("AUTH_MODE must be either 'device' or 'application'.")

    tenant_id = str(settings.get("AZURE_TENANT_ID", "")).strip()
    client_id = str(settings.get("AZURE_CLIENT_ID", "")).strip()
    secret = str(settings.get("AZURE_CLIENT_SECRET", "")).strip()
    hostname = str(settings.get("SPO_HOSTNAME", "")).strip()
    site_path = normalize_site_path(str(settings.get("SPO_SITE_PATH", "/")).strip())
    ...
    if missing:
        raise ValueError(
            "Missing required settings in USER_SETTINGS: " + ", ".join(missing)
        )

    if auth_mode == "application" and not secret:
        raise ValueError(
            "AZURE_CLIENT_SECRET must be provided when AUTH_MODE is 'application'."
        )

    download_dir_raw = settings.get("DOWNLOAD_DIR", "./aspx_dump")
    export_csv_raw = settings.get("EXPORT_CSV", "./aspx_index.csv")

    download_dir = (
        download_dir_raw
        if isinstance(download_dir_raw, pathlib.Path)
        else pathlib.Path(str(download_dir_raw))
    )
    export_csv = (
        export_csv_raw
        if isinstance(export_csv_raw, pathlib.Path)
        else pathlib.Path(str(export_csv_raw))
    )
    ...
    return {
        ...
        "DOWNLOAD_ASPX": coerce_to_bool(settings.get("DOWNLOAD_ASPX", True), default=True),
        "DOWNLOAD_DIR": download_dir,
        "EXPORT_CSV": export_csv,
        "SEARCH_FALLBACK": coerce_to_bool(settings.get("SEARCH_FALLBACK", True), default=True),
    }
...
if __name__ == "__main__":
    try:
        config = build_config_from_user_settings(USER_SETTINGS)
